...
**Created:** 2026-05-18
**Last amended:** 2026-05-26 (v6.0 Phase 0 instrumentation + Phase 1 quick-wins shipped. FX-054 fill-detection now traced via [FILL_DETECT_TRACE]/[FILL_WRITE] log lines (commit `bd5a54e`); FX-055 wallet reconciler re-wired into simple_oversight (`3704cd7`); FX-056 extreme-price filter live in SimpleAllocator (`80bd299`); FX-039 fill_type label fixed end-to-end + surfaced and closed a latent partial-fill alert bug (`9164f1f`). FX-051/052/053 (ROI tracker + OverCommitAllocator) remain open per the v6.0 sequencing plan. FX-054 root-cause investigation pending production trace capture.)
...
**Repo HEAD as of last amend:** `9164f1f` on `main` (FX-039 fill_type passthrough + partial-fill alert fix).
...
**v6.0 Phase 0 + 1 status (post-2026-05-26 ship):**
...
- FX-051 / FX-052 / FX-053 — still open per v6.0 sequencing (Phase 2 + Phase 3)

Bot remains halted; safe to restart after FX-054 root-cause shipped.)*

| ID | Title | Severity | Status | Tags |
|---|---|---|---|---|
| **FX-051** | **No loss-aware feedback / per-market ROI tracking.** SimpleAllocator ranks markets by `daily_rate × q_share` only. No input from `unwinds.pnl` per market. Bot re-deploys on markets that just produced losses. The 2026-05-25 kill switch event saw same-market repeat losses on `0x46c09232` (still in alloc post-loss). **Violates Ground Rule 3.** Fix: per-market rolling ROI tracker + decision policy that cools markets with negative rolling ROI. See §3 detail. | **Critical (Ground Rule 3 violation)** | Open | `[ARCH]` `[BUG]` |
