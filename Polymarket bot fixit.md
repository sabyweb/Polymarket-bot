...
| **FX-075** | **positions-table persist failure is silent.** `save_all_positions` swallows write errors at `log.debug` (`database.py` + `state.py`). Same silent-failure class FX-054 fixed for `fills`, left unfixed for `positions`. A successful `fills` row + a failed positions write → on restart the bot reloads stale positions, losing the share update (and the cost basis FX-066 depends on). Fix: truthful return + WARNING, mirror FX-054. | Medium | Open | `[BUG]` |
...
- **Provenance:** 6 parallel research agents traced the fill → dump → accounting → learning → safety pipeline in full (`order_lifecycle.py`, `dump_manager.py`, `reward_farmer.py`, `simple_allocator.py`, `simple_oversight.py`, `market_roi_tracker.py`, `decision_policy.py`, `database.py`, `state.py`, `oversight/wallet_reconciliation.py`). Every HIGH-severity finding was re-verified by the lead against code (file:line in each §2 row). Trigger: the 2026-05-29 `EXPECTED_FILL_COST_FRAC` A/B silently no-op'd (FX-063), which prompted "are there other documented-but-unwired bugs?" → this audit.
- **Why grouped:** all 15 share one property — **none are exercised in dry mode; all activate at Stage C live cutover when real fills begin.** They converge in the same regime (live fills at 5× overcommit, 124 markets) and several compound (FX-065 corrupts cost basis → FX-066 mis-signs pnl → FX-067/FX-074 fail to surface it). Per-item symptom + file:line + fix direction are in the §2 table.
- **Remediation order (gates Stage C):**
  1. **CRITICAL — capital-protection holes (fix before any live capital): ✅ ALL SHIPPED 2026-05-29.**
     - FX-068 (oversight kill never halts farmer) — ✅ `c44e8f0`, 7 tests.
     - FX-066 (vwap_cost=0 → loss-as-profit) — ✅ Tier 1 floor `6d2b57c` + Tier 2 cost-basis-at-registration `c606e78`, 11 tests. (Residual: true no-fills orphan magnitude → FX-074.)
     - FX-065 (positions double-count) — ✅ `4686b6b`, 4 tests.
     - FX-067 (`log_unwind` silent/non-idempotent) — ✅ `8851416` (mirrors FX-054), 6 tests.
  2. **HIGH — detection + slippage (fix before sustained live): ✅ ALL SHIPPED 2026-05-30.**
     - FX-063 (oversight hot-reload) — ✅ `29e33e1`, 2 tests.
     - FX-069 (fill-rate kill defeated by 180s prune) — ✅ `29e33e1` (new `kill_fill_times` + `RF_KILL_FILL_HISTORY_SECS`), 5 tests.
     - FX-071 (no dump-time slippage cap) — ✅ `d8e6b00` (cost-basis slippage floor `RF_DUMP_MAX_SLIPPAGE_FRAC`; audit metric was backwards), 4 tests.
     - FX-072 (drift sweep misses fast buy→dump) — ✅ `288b3c4` (3-gate masked-fill add-back; supersedes the audit's snapshot fix), 6 tests.
  3. **MEDIUM — config headroom + correctness (fix during P6/P7 or accept with mitigation):**
     - FX-070 (dead is_dumping guard) — ✅ `29e33e1`, 3 tests.
     - FX-074 (WALLET_DESYNC halt-vs-observe — operator decision) — ✅ `29e33e1`, decision = observational + Discord page (`alerts.alert_wallet_desync`, no halt), 4 tests.
     - FX-073 (notional headroom at 5×) — ⚠️ PARTIAL: relieved by the EV-gate→0.015 retune (`29e33e1`) pulling the operating point off the 5.0× soft block; guardrail pinning at 5.0× unchanged.
     - Still open: FX-075 (positions persist silent), FX-076 (FX-060 inert lever), FX-064 (cold-start doc/knob), FX-077 (reward HMAC — verify first).
- **Acceptance criterion:** CRITICAL set shipped + tested before Stage C live cutover; HIGH set shipped or explicitly accepted-with-mitigation; MEDIUM set tracked. Re-run the relevant agent trace post-fix to confirm closure.
- **Risk profile:** documentation-only at this stage (no code change in this entry). The fixes themselves are individually small and self-contained; each will ship as its own single-axis commit (P3) referencing its FX-ID.
- **Related:** FX-054 (fill-detection — FX-072 is its residual gap; FX-067/FX-075 extend its truthful-write pattern), FX-049/FX-055 (wallet reconciler — FX-074), FX-056 (extreme-price filter — FX-071 is its dump-time gap), FX-060 (FX-076), FX-058 (FX-073 retunes its thresholds for the actual 5× operating point), ground_rules.md Rule 3 (FX-076 = an expansion trigger that pulls a non-binding lever).
- **History:**
...
### FX-065 + FX-066 + FX-067 + FX-068 — pre-Stage-C CRITICAL live-fill set [FIXED]

- **Severity:** all Critical (capital-protection / loss-accounting integrity) except FX-065 High
- **Status:** Fixed 2026-05-29 (5 commits, each tested, 0 regressions); staged in `main`, NOT yet on Helsinki (live-only behaviors — land at the Stage C `git pull`)
- **Tags:** `[BUG]` `[SAFETY]`
- **Opened:** 2026-05-29 (pre-Stage-C live-fill readiness audit — 6 agents, see the FX-063…077 umbrella in §3)
- **Why grouped:** all four are loss-detection / kill-enforcement holes that converge in the live-fill-at-5×-overcommit regime and several compound (FX-065 corrupt cost basis → FX-066 mis-signed pnl → FX-067/FX-074 fail to surface it). Per-item symptom + file:line + fix direction are in the §2 FX-063…077 rows.
- **FX-068 (`c44e8f0`) — oversight kill switch now halts the farmer.** `simple_allocator.check_kill_switch` wrote `kill_switch=True` + `deploys=[]` to alloc JSON (24h loss >10% wallet / 15% drawdown), but `reward_farmer._load_allocations` only read `action=="deploy"` rows → the kill degraded to "empty deploys → stop new orders"; existing orders not cancelled, sticky halt not engaged. Fix: capture the alloc `kill_switch` flag in `_load_allocations`; `run_cycle` calls `_activate_kill_switch("oversight:<reason>")`. 7 tests (`tests/test_fx068_oversight_kill.py`).
- **FX-065 (`4686b6b`) — no positions double-count on a duplicate `handle_fill`.** `record_fill` ran before AND outside the `log_fill` INSERT-OR-IGNORE idempotency boundary → a re-handled fill double-counted shares + corrupted VWAP. Fix: `database.fill_event_exists()` guards `record_fill` with the same key the `fills` table uses. 4 tests (`tests/test_fx065_fill_idempotency.py`).
- **FX-067 (`8851416`) — `log_unwind` truthful + idempotent** (mirrors FX-054). Pre-fix it swallowed write errors at `log.debug` + returned None → a dropped row removed a loss from the kill math (the 2026-05-25 "8 SELLs, 1 unwinds row" shape); also non-idempotent (restart double-log). Fix: `unwinds.unwind_event_id` column + partial unique index + `INSERT OR IGNORE` + truthful bool + WARNING on error; `check_dump_fills` keys by dump order id. 6 tests (`tests/test_fx067_unwind_idempotency.py`).
- **FX-066 (`6d2b57c` + `c606e78`) — unknown-cost dump never recorded as profit.** Orphan/recovered positions are registered from on-chain balance via `set_shares` (no price) → `get_avg_price=0` → `vwap_cost=0` → `pnl = +sell_revenue` (loss recorded as profit → excluded from `SUM(pnl WHERE pnl<0)` → kill blind). **Tier 1 (`6d2b57c`):** floor `vwap_cost` to the gross proceeds when cost basis is unknown → `pnl = −fee ≤ 0` (never profit). **Tier 2 (`c606e78`, Option G):** reconstruct `avg_price` at orphan registration from the `fills` the scan already pulls (`database.fills_vwap` + optional `set_shares(avg_price=…)`), fixing dump pnl AND per-market ROI AND notional-guardrail visibility at the source. 11 tests (`tests/test_fx066_tier1_loss_floor.py`, `tests/test_fx066_tier2_cost_basis.py`). **Residual:** true no-fills orphans (no local cost basis) still record only the Tier-1 floor magnitude → that's FX-074 (make WALLET_DESYNC act).
- **Verification:** 28 new tests; 143 in the broad regression sweep (FX-06x + startup-recovery + positions/db/order-lifecycle/dump/capital/farmer/fill-detection). Zero regressions.
- **Deployment:** all in `main` (`c44e8f0`→`c606e78`); shadow run on Helsinki continues on the pre-fix code (these don't fire in dry mode); the CRITICAL set lands at the Stage C cutover pull.
- **Related:** FX-054 (FX-067/FX-072 extend its truthful-write + drift work), FX-049/FX-055/FX-074 (wallet reconciler), FX-056/FX-071 (extreme-price), FX-051 (ROI tracker reads the unwinds these fixes correct), ground_rules.md Rule 3.
- **History:**
  - 2026-05-29 — opened via the live-fill audit; CRITICAL tier implemented + shipped same day per operator instruction "(c): document the full set, then fix the CRITICAL gaps before Stage C." FX-066 implemented as the operator-chosen tiered build (Tier 1 safety floor + Tier 2 source fix; Tier 3 = FX-074, deferred to operator decision).

---

### FX-063 + FX-069 + FX-070 + FX-071 + FX-072 + FX-074 — pre-Stage-C HIGH set + 2 operator decisions [FIXED]

- **Severity:** mixed — FX-063/069/071/072 High, FX-070/074 Medium (detection + slippage + dump-loss + config holes that all activate at live cutover)
- **Status:** Fixed 2026-05-30 (3 commits, 24 tests, 0 regressions); staged in `main`, NOT yet on Helsinki (live-only behaviors — land at the Stage C `git pull`)
- **Tags:** `[BUG]` `[ARCH]` `[SAFETY]` `[OPS]`
- **Opened:** 2026-05-29 (pre-Stage-C live-fill readiness audit — 6 agents, see the FX-063…077 umbrella in §3)
- **Why grouped:** the HIGH tier of the live-fill audit plus the two MEDIUM items resolved alongside it (FX-070 one-line guard fix; FX-074 operator decision) — none fire in dry mode, all converge in the live-fill-at-5×-overcommit regime, and they ship as the second wave after the CRITICAL set.
- **FX-063 (`29e33e1`) — oversight hot-reloads `config_overrides.json`.** `simple_oversight.run_once` now calls `BotConfig.instance().check_and_reload()` each cycle, so the allocator's 8 cfg knobs no longer freeze at process-startup (the runbook's "tune via config_overrides.json" table is now functional without a restart). 2 tests (O13/O14).
- **FX-069 (`29e33e1`) — fill-rate kill no longer defeated by the 180s prune.** New `MarketState.kill_fill_times` (flat list, pruned to 6h via new knob `RF_KILL_FILL_HISTORY_SECS=21600`) is fed in `handle_fill` and read by `_guardrail_fill_rate_ratio`; the 1h-vs-6h trend kill now sees real history instead of degenerating to "≥5 fills/180s". `can_place`'s 180s `fill_times` breaker buffer is untouched. 5 tests.
- **FX-070 (`29e33e1`) — dead `is_dumping` guard fixed.** `_sync_exchange_positions` now checks `ms.dump_state` / `ms.dump_orders` instead of the always-False `hasattr(dump_mgr, "dump_states")`, so a position that is actively dumping is no longer `remove_market`'d mid-dump. 3 tests.
- **FX-071 (`d8e6b00`) — dump-time slippage floor against cost basis.** `dump_position` floors the SELL to `cost × (1 − RF_DUMP_MAX_SLIPPAGE_FRAC)` (new knob, default 0.05; cost = `state["fill_price"]`) — a bounded-loss exit. The abandon-timer holds if the book won't meet the floor, and the floor applies only when cost basis is known. No extra RPC. 4 tests.
- **FX-072 (`288b3c4`) — fast buy→dump masked-fill recovery.** 3-gate add-back in `_reconcile_balance_drift`: (1) an outstanding dump captured at cycle top via new `capture_pre_cycle_dumps` (free, no RPC); (2) that dump order absent from the cycle-top `open_ids` snapshot (drained, not a reprice-cancel); (3) `check_dump_fills` did NOT record the unwind this cycle (new `fx072_unwound_this_cycle` flag). Recovers the 2026-05-25 burst (9 buys→8 dumps→1 fill) and routes through `handle_fill` with the bucketed `drift:` event_id so FX-065/FX-067 idempotency holds. 6 tests (reproduction fails pre-fix, passes post-fix; mutation-checked).
- **FX-074 (`29e33e1`) — WALLET_DESYNC operator decision: observational + page.** New `alerts.alert_wallet_desync` (Discord) wired fail-open into `reconcile_wallet_invariant`'s desync branch. Per operator decision it stays OBSERVATIONAL — no halt, no allocation gating — but now pages instead of only logging to journalctl. 4 tests.
- **EV-gate decision (`29e33e1`) — `RF_OVERCOMMIT_EXPECTED_FILL_COST_FRAC` default 0.02 → 0.015** in `config.py` (lands the overcommit ratio inside the 3-8× band without sitting on the 5.0× soft block). ⚠ Helsinki's `config_overrides.json` pins 0.01 (overrides the default) — it must be removed at the Stage-C pull or the new default no-ops.
- **Audit-spec corrections:** two of the audit's fix-specs were wrong and were corrected in review. **FX-071** — the audit's metric was backwards (it compared the SELL limit to the best bid; a limit *below* the bid simply fills at the better price), so the shipped fix instead floors the executable price against the known cost basis. **FX-072** — the audit's snapshot fix was ineffective cross-cycle and cost ~250 RPC/cycle; a follow-up invariant retry an agent proposed also missed a partial-concurrent-fill case. The shipped 3-gate add-back supersedes both.
- **Verification:** 24 new tests across the 6 items; targeted regression sweep green throughout (oversight / order-lifecycle / dump / farmer / fill-detection / wallet-reconcile). Zero regressions.
- **Deployment:** all in `main` (`29e33e1`→`288b3c4`; docs at `0876078`); shadow run on Helsinki continues on the pre-fix code (these don't fire in dry mode); the set lands at the Stage C cutover pull (remove the `config_overrides.json` 0.01 EV pin then).
- **Related:** FX-054 (FX-072 closes its residual drift gap), FX-049/FX-055 (wallet reconciler — FX-074), FX-056 (extreme-price filter — FX-071 is its dump-time counterpart), FX-058/FX-073 (FX-073 partially relieved by this EV-gate retune), the CRITICAL-set block above (FX-065/066/067/068), ground_rules.md Rule 3.
- **History:**
  - 2026-05-30 — HIGH set + FX-070/074 + the two operator decisions implemented and shipped per the same "(c)" instruction that drove the CRITICAL set; 3 single-axis commits, 24 tests, 0 regressions. `docs/STATUS_2026-05-29.md` is the authoritative session record for this work.
