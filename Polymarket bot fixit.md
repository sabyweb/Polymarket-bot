...
**Created:** 2026-05-18
**Last amended:** 2026-05-26 (v6.0 Phase 2 shipped. FX-051 per-market ROI tracker + cooldown decision policy live (`e4f2ee3`, +1915 LOC, 29 new contract tests + 1 integration test). Bot now self-corrects on per-market losses: ROI < -5% with ≥3 samples OR fill_loss ≥ $2 in a single event → 24h cooldown → market excluded from allocator deploy set. Ground Rule 3's mandatory auto-correction loop is now implemented (data layer + decision layer + integration wiring). Bot remains halted; next step is adversarial breakage review of the FX-051 system before unhalting.)
...
**Repo HEAD as of last amend:** `e4f2ee3` on `main` (FX-051 ROI tracker + decision policy + 4 new DB tables + integration into simple_oversight + SimpleAllocator).
...
**v6.0 Phase 0 + 1 + 2 status (post-2026-05-26 ship):**
...
- **FX-051 — SHIPPED (`e4f2ee3`); per-market ROI tracker + cooldown decision policy. Ground Rule 3 self-correction loop now operational. 29 contract tests + 1 end-to-end integration test. Bot remains halted pending adversarial review.**
- FX-052 / FX-053 — still open (Phase 3 OverCommitAllocator; gated by FX-051 review + farmer-side kill-threshold retune)

Bot remains halted; restart blocked on adversarial review of FX-051 + FX-054 root-cause.)*
