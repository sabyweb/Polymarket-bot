...
**Created:** 2026-05-18
**Last amended:** 2026-05-28 (FX-054 shipped — root-cause fix for fill-detection under-counting, after adversarial audit. Three-axis fix in `database.py` + `order_lifecycle.py`: (F1) idempotent log_fill with partial unique index on fill_event_id + truthful return value + honest [FILL_WRITE] log step; (F2) phantom-check fail-OPEN within 60s balance-lag tolerance window after slot.placed_at (closes the silent-zero of legitimate fills during Polygon CTF transfer confirmation); (F3) end-of-cycle drift catch-up sweep on (cids_processed - primary_handled) that compares on-chain CTF to tracked position and writes catch-up rows with 5-min-bucketed event_ids. 14 new adversarial tests in `tests/test_audit_fill_detection.py` across 4 attack families (idempotency, lag tolerance, drift catch-up, stacked-failure invariants); audit caught a real bug in the drift sweep (passing None into NOT NULL fills.order_id → INSERT OR IGNORE silent drop) which is now closed by defensive None→'' coercion. 227 tests pass across FX-054 + FX-057 + FX-051 + adjacent suites; zero regressions. Bot's restart blocker on FX-054 is now closed at the LOGIC layer.)
...
**Repo HEAD as of last amend:** `d2f74f7` on `main` (FX-057); new commit pending for FX-054 root-cause fix.
...
**v6.0 Phase 0 + 1 + 2 + 3 status (post-2026-05-28 ship):**
- **FX-054 — SHIPPED (pending commit); 3-axis root-cause fix (idempotent log_fill + balance-lag tolerance in phantom check + end-of-cycle drift catch-up sweep). 14 adversarial tests in tests/test_audit_fill_detection.py. Restart blocker LOGIC closed; production trace confirmation still recommended on next operational run.**
...
- FX-052 / FX-053 — still open (Phase 3 OverCommitAllocator; gated by FX-051 review (closed via FX-057) + farmer-side kill-threshold retune)

Bot's restart blockers are now closed at the logic + accounting layers. Production trace capture on next operational run remains recommended to verify FX-054 against real-fill-shape data.)*
