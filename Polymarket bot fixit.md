...
- **Remediation order (gates Stage C):**
  1. **CRITICAL — capital-protection holes (fix before any live capital):**
     - FX-068 (oversight kill never halts farmer) — ~5 LOC, highest leverage.
     - FX-066 (vwap_cost=0 → loss-as-profit) + FX-065 (positions double-count) — the loss-accounting integrity pair.
     - FX-067 (`log_unwind` silent/non-idempotent) — mirror FX-054.
