...
| **FX-069** | **Fill-rate spike kill defeated by 180s prune.** `can_place` mutates `ms.fill_times[s] = [t for t in … if now-t < 180s]` every placement attempt (`order_lifecycle.py:1068`); the fill-rate kill reads the SAME dict over 1h/6h windows (`reward_farmer.py:1359`). For actively-quoted markets the 6h baseline can't accumulate past 180s → kill degenerates from a 1h-vs-6h trend detector to "≥5 fills/180s", blind to slow steady bleed. The 2026-05-25 kill fired only because fills clustered within one 180s window. Fix: separate the breaker's prune buffer from the kill's history (e.g. use the existing `_global_fill_times` or a non-pruned store). | High | Open | `[BUG]` `[SAFETY]` |
| **FX-070** | **Dead `is_dumping` guard strands mid-dump positions.** `reward_farmer.py:774-776` gates on `hasattr(self.dump_mgr, "dump_states")`, but `DumpManager` has no such attribute (uses per-market `ms.dump_state`) → always False → `is_dumping` always False → `_sync_exchange_positions` can `remove_market` a position that is actively dumping. Fix: check `ms.dump_state` / `ms.dump_orders` instead. One line. | Medium | Open | `[BUG]` |
| **FX-071** | **No dump-time slippage / extreme-price cap.** FX-056's `[0.10, 0.90]` filter is allocation-time AND fail-open (no price hint → midpoint 0.5 → passes). Nothing throttles a dump on a market that drifted to / was deployed at an extreme price. The 13.3%-slippage class (2026-05-25, $0.08→$0.07) is unmitigated at dump time. Fix: add a dump-time extreme-price / max-slippage guard in `dump_manager`. | High | Open | `[ARCH]` |
| **FX-072** | **F3 drift catch-up sweep can't recover fast buy→dump.** The end-of-cycle sweep compares on-chain balance to tracked shares, but dumps run earlier in the same cycle (inside `handle_fill`) and remove the on-chain evidence → `drift < 1.0` → no catch-up. The exact 2026-05-25 burst shape (9 buys → 8 dumps, 1 `fills` row) is therefore NOT fully closed — consistent with FX-054's own "production verification still recommended." Fix: record the fill at detection time regardless of subsequent dump, or snapshot balance pre-dump. | High | Open | `[BUG]` `[ARCH]` |
| **FX-073** | **Notional guardrail headroom collapses at the 5× operating point.** Soft block `RF_MAX_NOTIONAL_RATIO=5.0` EQUALS the 5× design target (observed parked at exactly 5.00× post-A/B) → ~half of natural fluctuation lands >5.0× → chronic intermittent placement throttle. Rapid-growth kill `RF_RAPID_GROWTH_KILL_RATIO=5.0`/300s false-fires on a cascade/realloc dip-then-refill within one window (e.g. 0.9×→5.5× = 6.1× > 5.0× → sticky halt of a healthy bot). Fix: raise soft block to ~6.5 and rapid-growth ratio to ~7-8 for sustained 5× live, or rethink rapid-growth as absolute-delta not ratio. | Medium | Open | `[ARCH]` |
| **FX-074** | **WALLET_DESYNC is observational-only — never halts.** The FX-049 reconciler emits `[CRITICAL] WALLET_DESYNC` on >$0.50 cash drift but keeps trading (`simple_oversight.py:189-190` "the reconciler is observational, never gates allocation"). It is the designated backstop for FX-066/FX-067 (missed losses surface as cash drift) but only logs — nothing acts without a human watching journalctl. Decision needed: should desync halt, or stay observational with external alerting? | Medium | Open | `[ARCH]` `[SAFETY]` |
...
- **Remediation order (gates Stage C):**
  ...
  2. **HIGH — detection + slippage (fix before sustained live):**
     - FX-069 (fill-rate kill defeated by 180s prune), FX-072 (drift sweep misses fast buy→dump), FX-071 (no dump-time slippage cap), FX-063 (oversight hot-reload).
  3. **MEDIUM — config headroom + correctness (fix during P6/P7 or accept with mitigation):**
     - FX-073 (notional headroom at 5×), FX-070 (dead is_dumping guard), FX-075 (positions persist silent), FX-076 (FX-060 inert lever), FX-064 (cold-start doc/knob), FX-077 (reward HMAC — verify first), FX-074 (WALLET_DESYNC halt-vs-observe — operator decision).
