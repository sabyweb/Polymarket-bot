...
**Created:** 2026-05-18
**Last amended:** 2026-05-27 (FX-057 shipped — adversarial audit of FX-051 found 7 bugs across 2 attack families (cold-start trap, cooldown gaming). All 7 reproduced in `tests/test_audit_cooldown_logic.py`. 5 targeted fixes closed every finding: fast-path threshold $2 → $1; sample gate 3 → 1; re-cool on expired-but-still-bad cooldown; bound ROI when capital_avg < $0.10; lookback-before-window for capital_committed_avg integration. Bot's loss-avoidance behaviour now consistent with ground-rules-target rate of <1 fill/day. 72 tests pass across FX-051 + audit suites.)
...
**Repo HEAD as of last amend:** `e4f2ee3` on `main` pre-FX-057; new commit pending for FX-057 retune + audit tests + tracker math fix.
...
*(2026-05-26: ground rules established in `ground_rules.md`. SimpleAllocator path (`0fafa1b`) deployed 2026-05-25, kill-switched 3h27m later on fill-rate spike (lost $26 trade pnl while earning $4.78 rewards). Six new FX-NNN entries (FX-051 through FX-056) opened.

**v6.0 Phase 0 + 1 + 2 status (post-2026-05-27 ship):**
- FX-054 — in progress; instrumentation shipped (`bd5a54e`), root cause pending production trace
...
- **FX-057 — SHIPPED (pending commit); adversarial audit of FX-051 closed 7 found bugs (cold-start trap × 4, cooldown gaming × 3). Retunes thresholds for ground-rules-target operating regime + adds re-cool semantics + tracker math fix. 7 new audit tests; 72 total pass.**
- FX-052 / FX-053 — still open (Phase 3 OverCommitAllocator; gated by FX-051 review (now closed via FX-057) + farmer-side kill-threshold retune)

Bot remains halted; restart blocked on FX-054 root-cause (fill detection accuracy).)*
...
| ~~**FX-057**~~ | ~~**FX-051 cooldown thresholds + lifecycle gaps surfaced by adversarial audit.**~~ ✅ SHIPPED (pending commit, 2026-05-27). 7 attack scenarios written in `tests/test_audit_cooldown_logic.py` exposed: (CS-1/2/CG-2/3) slow-bleed markets under the $2 absolute fast-path AND under the 3-sample roi gate never cooled; (CS-3) capital_avg=0 produced misleading -100 ROI in [LEARN] telemetry; (CS-4) `_capital_committed_avg` undercounted when only late-window snapshots existed; (CG-1) expired cooldown + still-bad ROI reactivated for one farmer cycle before re-cooling. 5 targeted fixes: ABS_LOSS_FAST_COOLDOWN_USD 2.0→1.0, ROI_COOLDOWN_MIN_SAMPLES 3→1, `evaluate_market` re-cools on expired+still-bad, tracker bounds ROI to 0 when capital_avg<$0.10, `_capital_committed_avg` extrapolates from latest snapshot before window. All 7 audit tests pass after fix; full 72-test sweep across FX-051 + adjacent suites green; 83 more tests pass across capital/dump/DB/wallet/oversight. See §4 detail. | High → CLOSED | Fixed (pending commit) | `[ARCH]` `[BUG]` |
| **FX-052** | **SimpleAllocator caps total notional below wallet (anti-overcommit).** `DEPLOY_RATIO = 0.95` + `MAX_PER_MARKET_USD = 60` + `MAX_DEPLOYED_MARKETS = 20` collectively cap notional at $1200, when Polymarket's overcommit allowance permits 3-8× that (= $3.6k-$9.6k notional on a $1.2k wallet). **Violates Ground Rule 2.** Memory file `project_capital_overcommit.md` already documents the underlying mechanism — the deployed code ignored it. Fix: replace with per-market `min_size` allocation + target market count 100-200 + remove total-notional cap. See §3 detail. | **Critical (Ground Rule 2 violation)** | Open | `[ARCH]` |
| **FX-053** | **Hard-coded market count cap leaves 5000+ reward markets on the table.** `MAX_DEPLOYED_MARKETS = 20` + `MIN_EXPECTED_PER_MARKET = $0.01` filter trims a 5,235-market reward pool to 20. Aggregate strategy (sum sub-$1 accruals across many markets) requires being on 100-500 markets at `min_size`, not 20 markets at $60. **Violates Ground Rule 1.** Fix: drop the count cap, accept smaller per-market expected reward, deploy on every market where `daily_rate × q_share > expected_fill_cost`. See §3 detail. | **Critical (Ground Rule 1 violation)** | Open | `[ARCH]` |
| **FX-054** | **Fill detection misses fills in high-frequency regime.** On 2026-05-25, on-chain data-api showed 9 TRADE BUYs in 3.5h totaling $844; bot's `fills` DB table contains only **1** row. 8 of 9 real fills are invisible to bot's accounting (and therefore to FX-049 reconciliation, I7 hourly_loss, any future ROI tracker). **Instrumentation shipped in `bd5a54e` (2026-05-26)** — every branch of `detect_fills` + `handle_fill` now emits `[FILL_DETECT_TRACE]` / `[FILL_WRITE]` log lines. Root-cause fix pending production trace capture on next operational run. **Without this fix, FX-051's ROI tracker has no input.** | **Critical (blocks Ground Rule 3)** | In Progress (`bd5a54e`) | `[BUG]` `[ARCH]` |
...
| **FX-045** | **Priority 1 q_share returns upper-bound heuristic, not measurement.** `_query_windowed_scoring` returns `scoring_ratio = our_scoring_snapshots / total`; resolver maps to `min(scoring_ratio × 0.5, 0.5)`. For any market where the bot's orders are 100% in-zone (typical post-FX-036), q_share collapses to the 0.5 max regardless of how many competitors are queued. Priority 1 trumps Priority 2 cumulative even when cumulative is healthy and non-poisoned. Live probe 2026-05-23: both deployed markets show q_share=0.5 (Priority 1) while cumulative ratio = 0.000249-0.000405 — a **~1500× overestimate** feeding I6 as `est_d`. Root cause of perpetual I6 SEVERELY firing and structural G3 unreachability. | **High (friend-rollout G3 blocker, structural)** | Open | `[BUG]` `[ARCH]` |
| **FX-046** | **Q-score formula candidate vs actual payouts diverges 24-94×.** `reward_tracker.q_score_order` uses `((max_spread − dist)/max_spread)² × size`. Live probe predicts ~$0.02-0.05/day from current 2 deployed markets; actual payouts averaged $1-5/day over recent observation window. Discrepancy could be (a) Polymarket reward formula ≠ squared-weight (architecture doc §4.23.1 actually says linear), (b) `market_q` over-counts competition, or (c) market state evolved between accrual and snapshot. Needs empirical reconciliation against historical book snapshots before code change. **Settles whether the FX-045 priority-swap will overshoot.** | Medium (gates FX-045 design) | Open | `[ARCH]` `[INVESTIGATION]` |
...
| **FX-043** | **`_total_capital` stamp missing → guardrails fail-open during 0-deploy alloc moments.** Observed on Helsinki 2026-05-21 19:50-19:54 UTC: alloc file had 0 deploys for ~5 min, `_guardrail_total_capital_from_alloc` returned None, `[GUARDRAIL] total_capital=null` → notional + cluster + 24h-loss-kill-switch checks ALL disabled fail-open for the duration. No harm done that time (zero activity), but the invariant "guardrails always armed in LIVE" was violated. Friend-rollout multiplier — recurring failure mode. | Medium | Open | `[BUG]` `[SAFETY]` |
