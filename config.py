...
RF_FILL_RATE_SPIKE_FACTOR: float = 3.0      # Fill-rate spike kill: short-window(1h) fill rate vs 6h baseline rate that flags a spike. Promoted from a hardcoded constant so it's tunable without redeploy.
RF_FILL_RATE_MIN_BASELINE: int = 8          # Require ≥ N fills in the 6h baseline before the spike ratio is computed (was 5). Higher = a near-idle canary's sparse baseline doesn't read an ordinary cluster as a huge spike.
RF_FILL_RATE_KILL_LOSS_FRAC: float = 0.005  # LOSS-GATE: a fill-rate spike only escalates to a KILL when short-window(1h) realized loss exceeds this fraction of total capital (0.5%). A spike on benign min_size fills that exit flat (loss below this) is logged, NOT killed. The 10%/24h realized-loss kill + 20% unrealized-loss kill remain the money backstops. Set 0 to revert to the legacy ratio-only kill.
...
RF_KILL_DRAWDOWN_FRAC: float = 0.15           # OVERSIGHT drawdown kill threshold (simple_allocator.check_kill_switch). Default mirrors the hardcoded KILL_DRAWDOWN_FRAC=0.15. Exists so a RECORDED, time-bounded operator loosening (ground_rules.md change log) can ride out a deliberately-accepted drawdown WITHOUT a redeploy. A falsy/invalid value falls back to 0.15 (you cannot disable the kill by setting 0). To honor an override you must ALSO raise RF_FARMER_DRAWDOWN_KILL_FRAC to match, else the farmer's oversight-silence backstop re-kills. The realized-loss (10%/24h) and fill-rate-loss kills are unaffected and remain armed.
RF_KILL_DRAWDOWN_FRAC: float = 0.15           # OVERSIGHT drawdown kill threshold (simple_allocator.check_kill_switch). Default mirrors the hardcoded KILL_DRAWDOWN_FRAC=0.15. Exists so a RECORDED, time-bounded operator loosening (ground_rules.md change log) can ride out a deliberately-accepted drawdown WITHOUT a redeploy. A falsy/invalid value falls back to 0.15 (you cannot disable the kill by setting 0). To honor an override you must ALSO raise RF_FARMER_DRAWDOWN_KILL_FRAC to match, else the farmer's oversight-silence backstop re-kills. The realized-loss (10%/24h) and fill-rate-loss kills are unaffected and remain armed.
RF_KILL_DAILY_REALIZED_LOSS_FRAC: float = 0.1  # FARMER realized-loss kill threshold. Operator can raise it temporarily to collect more data during A/B experiments; set back to 0.1 to restore default.
RF_KILL_PORTFOLIO_SOURCE: str = "db"          # FIX-3 (RC-5): source of the INVENTORY half of the drawdown/realized-loss kill's portfolio value (simple_oversight.run_once -> simple_allocator.check_kill_switch). "db" = legacy: cash + the `positions` table, which can (a) MISS an on-chain fill not yet written -> portfolio reads cash-only -> the 15% drawdown kill FALSE-trips and DEADLOCKS the farmer (2026-06-13: a $22 fill did exactly this), and (b) miss a held-to-resolution loss -> UNDER-fire (2026-06-12, RC-5). "onchain" = authoritative data-api /positions marks (sum of size*curPrice). Default "db" so the change is reversible/opt-in. Fail-safe (never silently disables NOR falsely fires the kill): on a data-api miss it reuses a fresh cached on-chain reading, else falls back to the "db" value with a WARNING.
RF_KILL_ONCHAIN_MAX_STALE_SECS: float = 3600.0  # FIX-3: when RF_KILL_PORTFOLIO_SOURCE="onchain" and the live data-api read fails, reuse the last good on-chain inventory value only if it is younger than this (~2 oversight cycles); beyond it, fall back to the DB-based value (logged WARNING).
...
RF_RAPID_GROWTH_MIN_BASELINE_RATIO: float = 0.5  # FX-087: the FX-058 rapid-growth (acceleration) kill (RF_RAPID_GROWTH_KILL_RATIO) only ARMS once the 300s-window MINIMUM notional_ratio is at least this — i.e. an established operating baseline. Below it (cold-start ramp from 0, a bounded canary holding <0.5x, or right after a kill-cancel) the max/min burst ratio is meaningless: 0 -> any positive reads as an enormous "burst". The dangerous high-exposure case is already bounded by the static soft/hard notional caps (RF_MAX/HARD_NOTIONAL_RATIO). Pre-FX-087 the code floored the window min to 0.0001 and FALSE-fired on the FIRST live placement after every restart — caught live on the very first canary order (0 -> 0.16x read as a 1578x burst, sticky-killing the farmer). 0 keeps it always-armed (legacy behavior; not recommended).
RF_COLD_START_Q_SHARE: float = 0.005  # FX-086 (closes FX-064): cold-start q_share prior for markets with no API q_share and no cumulative DB ratio — i.e. EVERY unseen market, which is the dominant population. The live EV gate's binding input was a HARDCODED module constant simple_allocator.COLD_START_Q_SHARE=0.005, not tunable at runtime (the documented RF_NEW_MARKET_Q_SHARE_PRIOR=0.10 is the LEGACY oversight_agent path only). FX-046 found the heuristics under-predict actual reward by 24-94×, so this prior is the single most impactful knob on capital efficiency, yet it could not be tuned without a code redeploy. Now cfg-driven; default unchanged at 0.005 (no behavior change on its own) so operators can recalibrate via config_overrides.json once live reward data lands. Compounds with RF_OVERCOMMIT_Q_SHARE_CONSERVATIVE_FACTOR (applied to all non-API q_share).
