...
## Current Production State (v6.6 — FX-083→089 shipped; bot LIVE on Helsinki as a cap-5 bounded canary with corrected accounting; local repo at ~/code)
...
|---|---|---|
| **Allocator (current path)** | `SimpleAllocator` class name retained for import-site compat; SEMANTICS transformed by FX-052+053 to OverCommitAllocator. No DEPLOY_RATIO, no MAX_PER_MARKET_USD, no hard 20-market cap. Per-market notional = cost-to-score `min_size × midpoint × 2 × (1 + buffer)`. Target band 50–200 markets simultaneous. Positive-EV gate: deploy iff `expected_reward × q_share > expected_fill_cost`. Soft sanity cap 500. New cfg knobs `RF_OVERCOMMIT_*`. | §4.15 (still describes legacy formula — needs body update), `simple_allocator.py`, `simple_oversight.py::run_once` |
...
| **Server deployment** | Hetzner CCX13 in Helsinki (`hel1`), Ubuntu 24.04 LTS, Python 3.14.4. **Verified 2026-05-31 (read-only SSH): Helsinki is at `3bb6137`, running `reward_farmer.py --mode dry` + `simple_oversight.py --loop` in P5 Stage B shadow (NOT halted, both services 0 restarts). The FX-063→082 + persistence fixes are on `main` but NOT yet pulled — they land at the Stage-C pull.** The local dev repo was relocated 2026-06-01 to `/Users/sabyasaachikarmakar/code/polymarket_bot` (out of TCC-protected ~/Downloads). Runbooks: `docs/runbooks/deployment_from_scratch.md` (provisioning), `9_of_10_p5_p7_operator_runbook.md` (cutover), `stage_c_pull_2026-05-31.md` (this pull). | §11.1–11.3; `docs/runbooks/` |
| **Wallet** | FUNDER `0xB23Bc80E6719099aeBE0c34389f05EC8C928503f` (Polymarket proxy). pUSD balance **$1,201.76 (verified live 2026-05-31 via read-only SSH; an ~$875 deposit landed ~2026-05-28; static in dry shadow)**. All V2 allowances unlimited, CTF approvals True. FX-049 wallet reconciliation invariant remains armed for next cycle after rebring-up. | `check_wallet.py`, `wallet_reconcile_history` table |
