...
**v6.6 scope (2026-06-01, later).** Live cutover + the canary post-mortem's two accounting fixes. Repo HEAD `dffb8fd`. Shipped after v6.5: **FX-083** production heartbeat/stall alert (farmer↔oversight, Discord); **FX-084** held-inventory unrealized-loss kill; **FX-085** `capital_efficiency` metric; **FX-086** cold-start q_share cfg-tunable (closes FX-064); **FX-087** rapid-growth kill no longer false-fires on the cold-start first placement (caught by the first live canary, which would otherwise self-kill on every live restart); **FX-088** reward sourcing moved to the public data-api `/activity` aggregate — the CLOB parser read key `"markets"` (API returns `"data"`) so `reward_earned`≡$0, blinding `capital_efficiency` AND the learning ROI (every market looked like a pure loss); **PROVEN on Helsinki: market_roi 24h reward $0→$10.12**; **FX-089** dump unwinds booked at the marketable execution price (best bid) not the order limit (recorded −$85 vs on-chain −$22; the source of the WALLET_DESYNC alarms). **Helsinki is now LIVE** — a bounded `--mode live` **cap-5 canary**, re-launched 2026-06-01 ~07:55 UTC on the corrected accounting (the learning loop cooled the prior losers + deployed fresh markets); ~$200 resting, kill_switch false, wallet flat $1184.53, **profitability pending the daily reward settlement (~00:20 UTC)**. Full suite **1116 pass**. Detail in `Polymarket bot fixit.md` (v1.35) + `docs/STATUS_2026-05-31.md` (Addenda 2–5). **NB: the v6.5 scope below is superseded on deployment state — the bot is no longer "Stage B dry shadow," it is a live canary.**

**v6.5 scope (2026-06-01).** Oversight persistence-bug cluster + two more fixes shipped; repo relocated; live-readiness eval. Repo HEAD `d81e978`. **FX-078** (`a264d72`) restored the 15% drawdown kill limb (oversight `portfolio_snapshots` writes had silently failed ~6 days on a NOT-NULL schema mismatch → `peak==wallet` → limb dead). **FX-080** (`8aa10f5`) added `_rollback_quiet()` across all ~30 `BotDatabase` writers (a no-rollback failed write had wedged the oversight connection — frozen reconcile + 95 MB WAL); closes FX-075. **FX-081** (`55f9b62`) fixed the silently-lost `__FILL_STORM__` audit marker (`db.execute_sql` never existed). **FX-082** (`d81e978`) is an *architectural* safety change: the 15% drawdown kill is **no longer oversight-only** — a farmer-side backstop (`_guardrail_oversight_silence_drawdown`) trips the farmer's own sticky kill on sustained oversight silence + live exposure + real drawdown, closing a single-point-of-failure on a kill limb. A 6-agent **live-readiness eval** returned **NOT-ready-for-live (~5.5/10)**; **self-learning ~1.5/6 EFFECTIVE** (the "6/6 wired" count is call-graph-only — no trigger has fired on a real outcome); **capital efficiency UNMEASURED** (the EV gate runs on a hardcoded 0.005 cold-start q_share that FX-046 says is 24–94× off). FX detail in `Polymarket bot fixit.md` (v1.31); session record in `docs/STATUS_2026-05-31.md` (Addendum). **Local dev repo relocated** to `/Users/sabyasaachikarmakar/code/polymarket_bot` (out of TCC-protected ~/Downloads so the Streamlit preview runs; Helsinki production unaffected). None of these are on Helsinki yet — they land at the Stage-C pull.
...
**Open before Stage C live cutover** (see `Polymarket bot fixit.md` §2 + the FX-063…077 umbrella entry): the HIGH set FX-063 (oversight hot-reload) / FX-069 (fill-rate kill defeated by the 180s `fill_times` prune) / FX-071 (no dump-time slippage cap) / FX-072 (drift sweep misses fast buy→dump), plus FX-070 (dead `is_dumping` guard) / FX-074 (WALLET_DESYNC paging) and the EV-gate retune (`EXPECTED_FILL_COST_FRAC` 0.02→0.015), are now **SHIPPED 2026-05-30** (commits `29e33e1` / `d8e6b00` / `288b3c4`); none of these gate Stage C. Remaining open — FX-073 (**PARTIAL — notional headroom: the live shadow ratio was hitting 7.5-7.9× at the 0.01 EV setting, near the 8.0× hard cap; the EV-gate→0.015 retune addresses this by right-sizing the gate rather than raising the caps, so the guardrail pinning at 5.0× is unchanged**), FX-076 (`global_reward_low` pulls a non-binding lever → inert), FX-064, FX-075, FX-077, FX-062.
...
| `RF_OVERCOMMIT_EXPECTED_FILL_COST_FRAC` | 0.015 | FX-052/053 EV gate slippage assumption (retuned 2026-05-30 `29e33e1`; Helsinki override 0.01 must be cleared at Stage-C pull) |
...
## Current Production State (v6.7 — FX-090/091/092 shipped: adverse-selection root-cause fix + capeff denominator + kill→Discord; bot LIVE on Helsinki as a cap-5 canary, NET still negative from news-market adverse fills; local repo at ~/code)

Updated 2026-06-02 (v6.7): the live canary exposed the REAL blocker — `SimpleAllocator` ranked by `daily_rate×q_share` → picked near-resolution / news markets the farmer refused (→ 0 orders, `0/5 on-book`) or that adversely filled (→ loss). Shipped + deployed: **FX-090** allocator time-to-event filter (farming resumed, `0/5`→`5/5 on-book`), **FX-091** capeff denominator ($78k→~$320; capeff ~1.4%/day), **FX-092** kill→Discord page, + config `RF_TARGET_QUEUE_AHEAD_USD`=4000 (deeper queue, round-1 adverse-fill cut). **Live HEAD `92ec34c`; gross capeff good but NET still negative (~−$25 since the FX-090 deploy) from news-market adverse fills — objective unproven; verdict at the ~00:20 UTC 06-03 settlement.** Full detail: `docs/STATUS_2026-06-02.md` + `docs/runbooks/live_canary_operator.md`. — Prior: updated 2026-06-01 later (v6.6: FX-083→089 + first live canary + halt + re-launch). **Deployment state: Helsinki is `--mode live` (cap-5 bounded canary), NOT dry shadow** — see the v6.6 scope above + `docs/STATUS_2026-05-31.md` Addendum 5. Earlier (v6.5: persistence cluster FX-078/080 + FX-081/082 + repo relocate). Rows where I'm uncertain about a fact are marked **(unverified)** rather than guessed; operator can confirm against Helsinki on next access. **v6.4 doc change:** legacy/rollback component sections (former §4.12 Bandit, §4.14 SafetyController, §4.15 β/η Continuous Allocator, §4.16 β/η Control System, §4.17 Patch-6–13 stack) consolidated into a single §4.14b; full detail preserved in git @ `c074446`.

| Layer | State | Location |
|---|---|---|
| **Allocator (current path)** | `SimpleAllocator` class name retained for import-site compat; SEMANTICS transformed by FX-052+053 to OverCommitAllocator. No DEPLOY_RATIO, no MAX_PER_MARKET_USD, no hard 20-market cap. Per-market notional = cost-to-score `min_size × midpoint × 2 × (1 + buffer)`. Target band 50–200 markets simultaneous. Positive-EV gate: deploy iff `expected_reward × q_share > expected_fill_cost`. Soft sanity cap 500. New cfg knobs `RF_OVERCOMMIT_*`. **FX-090 (2026-06-02): adverse-selection / time-to-event filter — enriches ranked candidates with `game_start_time`+`end_date_iso` (cached CLOB `/markets/{cid}`) and excludes markets within `RF_ALLOC_MIN_HOURS_TO_RESOLUTION`=48h / `RF_ALLOC_MIN_HOURS_TO_GAME_START`=12h (fail-open, backfill) so it stops proposing the near-resolution/news markets the farmer refuses; verified live `0/5`→`5/5 on-book`. Ranking is still raw `daily_rate×q_share`, so news markets still adversely fill — the remaining binding constraint.** | §4.15 (still describes legacy formula — needs body update), `simple_allocator.py`, `simple_oversight.py::run_once` |
| **Legacy allocator path** | `profit/allocator.py` (continuous β/η formula) + `oversight/allocation_writer.py` — present in repo but NOT in current production path. Activated only if operator reverts systemd ExecStart to `oversight_agent.py --loop`. Kept as rollback option; not actively maintained against v6.x changes. | `profit/allocator.py`, `oversight/allocation_writer.py` |
| **Self-learning loop** | 6/6 ground-rules §3 triggers WIRED to behavior change (was 0/6 pre-FX-051) — **but the 2026-06-01 live-readiness eval found only ~1.5/6 EFFECTIVE in the live cold-start regime: no trigger has ever fired on a real outcome (0 cooldown/recalibration/ROI rows in the production DB), #4 is inert (FX-076), #2 mis-calibrated ~24×. "Wired" ≠ "effective" — see fixit/STATUS.** FX-051 `market_roi_tracker` computes rolling 1h/24h/7d snapshots from `fills` + `unwinds` + `capital_committed_snapshots` + `/rewards/user/markets` API. `decision_policy.evaluate()` returns 5 behavior-change outputs: `excluded_cids` (cooldowns), `size_reduction_cids` (P4 fill_rate), `global_tighten` (P4 global_loss), `global_reward_low` (P10), `q_share_distrust_cids` (P11). Tracker output also persisted to `market_roi` table. **FX-091 (2026-06-02): the global `capital_efficiency`/`total_capital` scorecard in `get_global_summary` was garbage ($78k denom from summing per-market avgs that forward-fill dropped markets to `now`) → fixed to the time-averaged per-cycle committed capital from `capital_committed_snapshots`; capeff now reads ~1.4%/day gross (above target). Per-market roi/cooldowns untouched.** | `market_roi_tracker.py`, `decision_policy.py`, §4 (parts still describe legacy β/η — needs body update) |
| **Legacy learning loop** | Four scalars `capital_scale`, `reward_trust`, `β`, `η` — described in §4.13 / §4.16, present in `profit/learning.py`, NOT in current production path. `simple_oversight` doesn't read them. Kept as rollback option. | `profit/learning.py` |
| **SafetyController** | Present in `oversight/safety_controller.py` with 7 states + 14 invariants — used by the legacy `oversight_agent.py`, NOT by `simple_oversight.py` (current production path). simple_oversight uses a minimal kill switch on `simple_allocator.check_kill_switch` (24h loss > 10% wallet OR drawdown > 15%). **(unverified: whether any code path imports SafetyController from simple_oversight via a cross-call — operator should grep before considering SafetyController fully dormant.)** | §4.14, `oversight/safety_controller.py` |
| **Runtime guardrails (farmer-side)** | Post-FX-058 (P1 of 9/10 plan): soft block at `RF_MAX_NOTIONAL_RATIO=5.0` (was 2.0), hard cancel at `RF_HARD_NOTIONAL_RATIO=8.0` (was 2.5) — cfg-driven for runtime tuning. New acceleration-based kill: `_guardrail_rapid_notional_growth` kills if `notional_ratio max/min` over `RF_RAPID_GROWTH_WINDOW_SEC=300` exceeds `RF_RAPID_GROWTH_KILL_RATIO=5.0`. Cluster cap unchanged at 0.5·T. Kill switch on {24h loss > 0.1·T, CF < 0.01, fill-rate spike > 3×}. **FX-082 (2026-06-01) adds a farmer-side drawdown backstop (`_guardrail_oversight_silence_drawdown`): trips the kill when oversight is silent past `RF_OVERSIGHT_SILENCE_KILL_HOURS=2.0` (alloc mtime) AND live notional>0 AND a farmer-computed drawdown (`db.load_usdc_balance` vs new `db.get_wallet_peak_usd`) > `RF_FARMER_DRAWDOWN_KILL_FRAC=0.15` — so the 15% drawdown limb is NO LONGER oversight-only (closes a kill-limb SPOF). Fail-open on any missing signal.** `_total_capital` now stamped at alloc.json top-level metadata (FX-043 closed) so 0-deploy cycles don't silently fail-open. **FX-092 (2026-06-02): every kill now pages Discord via `alerts.alert_kill_switch` from `_activate_kill_switch` (the single chokepoint), fail-safe — a kill leaves the process alive-but-idle so the heartbeat/stale alert never covered it (the 2026-06-02 fill-rate kill went unnoticed until manual inspection).** | §4.18, `reward_farmer.py:_guardrail_check_and_log` |
| **Fill-detection (FX-054, P-pre-session)** | 3-axis defensive design: **F1** idempotent `log_fill` with partial unique index on `fill_event_id` + truthful bool return + honest `[FILL_WRITE]` log step. **F2** phantom-check fail-OPEN within `FILL_BALANCE_LAG_TOLERANCE_SEC=60` of `slot.placed_at` (closes Polygon CTF-confirmation-lag false phantoms). **F3** end-of-cycle drift catch-up sweep on `(cids_processed − primary_handled)` with 5-min-bucketed event_id idempotency. Symmetric SELL-side FX-007 still applies. **FX-072 (2026-05-30, `288b3c4`):** the F3 sweep (`_reconcile_balance_drift`) now also recovers a fast buy→dump masked fill via a 3-gate dump-drain add-back (captured-at-cycle-top + dump-order-absent-from-open_ids + unwind-not-recorded-this-cycle) — recovers the 2026-05-25 burst shape with zero added RPC; composes with the FX-065/FX-067 idempotency. **(unverified in production against a real fill burst — gate G-C pending operator P5/P6.)** | `order_lifecycle.py:detect_fills + _check_buy_phantom_fill + _reconcile_balance_drift`, `database.py:log_fill` |
| **Wallet reconciliation (FX-049 + FX-055)** | Runs once per `simple_oversight.run_once` cycle via `oversight/wallet_reconciliation.py::reconcile_wallet_invariant`. Compares actual wallet pUSD delta against expected (`Σ unwinds − Σ fills + data-api REWARD + MAKER_REBATE` since last baseline). `\|divergence\| > RF_WALLET_DESYNC_THRESHOLD_USD = $0.50` → `[CRITICAL] WALLET_DESYNC`. First-run baseline + fail-OPEN on data-api failure preserved from FX-049. FX-055 re-wired this into simple_oversight after the 2026-05-25 regression dropped it. **FX-074 (2026-05-30):** a `desync` now also fires a Discord page via `alerts.alert_wallet_desync` (observational — no halt, no allocation gating). | `oversight/wallet_reconciliation.py`, `simple_oversight.py:run_once`, `wallet_reconcile_history` table |
| **Execution modes** | `--mode {dry,shadow,live}` — unchanged. Default `dry`. Kill switch overrides mode. Operator runbook at `docs/runbooks/9_of_10_p5_p7_operator_runbook.md` documents the cutover. | §4.19, `reward_farmer.py:run`, systemd `ExecStart` line |
| **Telemetry** | Existing per-cycle `[CYCLE_SUMMARY]` + `[GUARDRAIL]` + `[CAPITAL_SOURCE]` unchanged. New v6.x lines: `[OVERCOMMIT_ALLOC]` (per allocator cycle: eligible/positive_ev/deploys/avoids/notional_total/overcommit_ratio/p4_size_reduction_cids/p4_global_tighten/p10_global_reward_low/p11_q_distrust_cids); `[LEARN]` + `[LEARN_COOLDOWN]` + `[LEARN_REACTIVATE]` + `[LEARN_WARN]` + `[LEARN_DIVERGENCE]` from decision_policy; `[FILL_DETECT_TRACE]` + `[FILL_WRITE]` + `[RECONCILE_DRIFT]` from FX-054. All structured / printf-style; machine-parseable. | §4.20 (still lists legacy lines — partial update needed) |
| **Oversight evaluator (current path)** | `simple_oversight.py::run_once` — 30-min cycle. Sequence: wallet probe → portfolio_snapshots write → peak/24h history reads → FX-055 wallet reconciliation → FX-051 tracker.tick + policy.evaluate → q_share divergence record (P11) → `allocator.compute()` with 6 inputs (wallet, peak, 24h_ago, realized_loss, + the 5 policy outputs above) → write alloc.json → telemetry. All in fail-open try/excepts so a single subsystem failure can't kill the cycle. **FX-063 (2026-05-30):** `run_once` now calls `BotConfig.instance().check_and_reload()` at cycle top (mirrors farmer/bot), so the SimpleAllocator's cfg knobs no longer freeze at oversight process start. | `simple_oversight.py:run_once` |
| **Legacy oversight evaluator** | `oversight_agent.evaluate(guard)` with flag-gated stages 1/2/3 — present in `oversight_agent.py`, not used in current production. Rollback path only. | `oversight_agent.py`, §4.21 |
| **Server deployment** | Hetzner CCX13 in Helsinki (`hel1`), Ubuntu 24.04 LTS, Python 3.14.4. **Verified 2026-06-02 (read-only SSH): Helsinki is at `92ec34c`, running `reward_farmer.py --mode live` (cap-5 canary) + `simple_oversight.py --loop`, both 0 restarts. config_overrides adds `RF_TARGET_QUEUE_AHEAD_USD:4000`; FX-090/091/092 all deployed. Live-canary ops: `docs/runbooks/live_canary_operator.md`; session detail: `docs/STATUS_2026-06-02.md`.** The local dev repo was relocated 2026-06-01 to `/Users/sabyasaachikarmakar/code/polymarket_bot` (out of TCC-protected ~/Downloads). Runbooks: `docs/runbooks/deployment_from_scratch.md` (provisioning), `9_of_10_p5_p7_operator_runbook.md` (cutover), `stage_c_pull_2026-05-31.md` (this pull). | §11.1–11.3; `docs/runbooks/` |
| **Wallet** | FUNDER `0xB23Bc80E6719099aeBE0c34389f05EC8C928503f` (Polymarket proxy). pUSD balance **~$1,159.76, flat (verified live 2026-06-02; peak $1,201.76; ~−$25 net since the FX-090 deploy from news-market adverse fills — see STATUS_2026-06-02)**. All V2 allowances unlimited, CTF approvals True. FX-049 wallet reconciliation invariant remains armed for next cycle after rebring-up. | `check_wallet.py`, `wallet_reconcile_history` table |
| **Tests** | **(verified this session)** 318 tests pass across the 9/10 plan + FX-051 + FX-054 + FX-057 + FX-045 + adjacent suites (`test_simple_allocator`, `test_simple_oversight`, `test_decision_policy`, `test_market_roi_tracker`, `test_audit_cooldown_logic`, `test_audit_fill_detection`, `test_p1`–`p11`, `test_p8_chaos_engineering`, `test_p9_integration_audit`, `test_capital_flow`, `test_wallet_reconciliation`, `test_dump_manager_fee`, `test_oversight_shadow`, `test_shutdown`). Zero regressions across these. **Full-suite count ~900** across 47 test files; 1 pre-existing failure in `test_simulation::TestScenarioDirections::test_over_aggressive_contracts_capital` (legacy `LearningController` path, predates this session, not introduced). | `tests/test_*.py` |
| **CI / build gate** | GitHub Actions `.github/workflows/test.yml` still configured. **(unverified: whether the CI workflow has been run against the most-recent commits this session; status badge on `README.md` was green at commit `06d8406` / 2026-05-24. Operator can check the badge after next push or trigger a manual run.)** | `.github/workflows/test.yml`, `README.md` |
| **9/10 plan gates** | **3 of 5 met at code level.** G-A (FX-052+053 OverCommitAllocator) ✓, G-B (4+ self-correction triggers wired — actually 6/6 now) ✓, G-D (FX-046 resolved/accepted) ✓. G-C (FX-054 production verify) ⏳ pending P6 operator runbook. G-E (G1 7-day clean run) ⏳ pending P7 operator runbook. **Honest current rating: 7.5/10.** Path to 9/10 = operator-driven live operation; no remaining architectural blockers. | `docs/runbooks/9_of_10_p5_p7_operator_runbook.md`, `Polymarket bot fixit.md` |
...
**Blockers:** none currently known. The v5.1.4 geoblock blocker is resolved by the Helsinki migration (see `CHANGELOG.md` v5.1.5 entry, item 1). The v5.1.5 bootstrap deadlock is resolved by `dd67f97`. Phase 0 housekeeping (FX-017 / FX-018 / FX-020) closed in v5.1.6. Open issues are tracked in detail in `Polymarket bot fixit.md` with stable `FX-NNN` IDs.
...
| `fill_rate_ratio` | 1h observed fill count / (6h baseline count × 1/6) over all markets' `ms.kill_fill_times` (FX-069: a separate non-pruned 6h history; `can_place` prunes `ms.fill_times` to 180s for the placement breaker, which previously starved this kill's baseline → blind to slow bleed) | unitless; requires baseline ≥ `MIN_FILL_BASELINE = 5` |
...
| Fill-rate spike | `1h-fill-count / 6h-baseline-rate > FILL_RATE_SPIKE_FACTOR = 3.0` AND `baseline ≥ MIN_FILL_BASELINE = 5` | `ms.kill_fill_times` across all markets (FX-069: a separate non-pruned 6h history; `can_place` prunes `ms.fill_times` to 180s for the placement breaker, which previously starved this kill's baseline → blind to slow bleed) |
...
**FX-071 (2026-05-30, `d8e6b00`) — dump-time slippage floor.** The FX-041 check above is a pre-fill *placement* heuristic; it does not bound how far below cost a *dump* SELL can crystallize once inventory exists. FX-071 closes that at dump time: when cost basis is known (`state["fill_price"]`), `dump_position` floors the SELL price at `cost × (1 − RF_DUMP_MAX_SLIPPAGE_FRAC)` (default 0.05), so a single dump never realizes more than the cap below cost. If the book won't meet the floor, `RF_DUMP_ABANDON_MINS` holds the position rather than crossing into a deeper loss (Rule 3). No extra RPC; only active when cost basis is known.
...
| `RF_DUMP_MAX_SLIPPAGE_FRAC` | 0.05 | FX-071: dump-time slippage floor. dump_position floors the SELL to cost*(1-this) (cost = state["fill_price"], CLOB) so one dump never crystallizes more than this below cost basis; if the book won't meet the floor, RF_DUMP_ABANDON_MINS holds the position (Rule 3). Only when cost basis known. 0 or ≥1 disables. Hot-reloadable. |
...
| `RF_KILL_FILL_HISTORY_SECS` | 21600 | FX-069: 6h prune horizon for MarketState.kill_fill_times — the fill-rate KILL-SWITCH history, SEPARATE from the 180s RF_FILL_BREAKER_WINDOW that can_place prunes for placement throttling. Must stay ≥ the 6h GUARDRAIL_FILLRATE_BASELINE_SECS or the slow-bleed kill regresses. |
...
|---|---|---|
| `29e33e1` + `d8e6b00` + `288b3c4` + `0876078` | **v6.3 — pre-Stage-C HIGH-set live-fill hardening (2026-05-30).** `29e33e1`: **FX-063** `simple_oversight.run_once` now calls `BotConfig.instance().check_and_reload()` each cycle so the SimpleAllocator's cfg knobs no longer freeze at oversight process start; **EV-gate retune** `RF_OVERCOMMIT_EXPECTED_FILL_COST_FRAC` 0.02→0.015 (lands the overcommit ratio inside the 3-8× band; Helsinki's `config_overrides.json` 0.01 pin overrides this — clear at Stage-C pull); **FX-069** new non-pruned `MarketState.kill_fill_times` (flat list, pruned to 6h via `RF_KILL_FILL_HISTORY_SECS = 21600`), fed in `handle_fill` and READ by `_guardrail_fill_rate_ratio` — NOT `ms.fill_times` (which `can_place` prunes to the 180s `RF_FILL_BREAKER_WINDOW`), fixing the kill being blind to slow bleed; **FX-070** `_sync_exchange_positions` is_dumping guard now checks `ms.dump_state`/`ms.dump_orders` (was a dead `hasattr(dump_mgr,'dump_states')`, always False); **FX-074** `[CRITICAL] WALLET_DESYNC` now ALSO fires a Discord page via new `alerts.alert_wallet_desync`, wired fail-open into the reconciler (observational — no halt, no allocation gating). `d8e6b00`: **FX-071** `dump_position` floors the dump SELL to `cost*(1 − RF_DUMP_MAX_SLIPPAGE_FRAC)` (new knob, default 0.05; cost = `state["fill_price"]`) so a single dump never crystallizes more than the cap below cost; if the book won't meet the floor, `RF_DUMP_ABANDON_MINS` holds the position. Only when cost basis known; no extra RPC. `288b3c4`: **FX-072** the FX-054 drift catch-up sweep (`_reconcile_balance_drift`) now recovers a fast buy→dump masked fill via a 3-gate dump-drain add-back (captured-at-cycle-top + dump-order-absent-from-open_ids + unwind-not-recorded-this-cycle); recovers the 2026-05-25 burst shape, zero added RPC, composes with FX-065/FX-067 idempotency. `0876078`: docs (`STATUS_2026-05-29` + fixit v1.27). 24 new tests; full fast tier **1031 passed**. Two audit fix-specs (FX-071/FX-072) were corrected in review. | **Pre-Stage-C HIGH set closed on `main`.** Bot is in P5 Stage B shadow (`--mode dry`) on Helsinki; these fixes are on `main` but **NOT yet pulled to Helsinki** — they land at the Stage-C pull. None gate Stage C. At that pull, also clear the Helsinki `config_overrides.json` 0.01 EV pin so the 0.015 retune binds. Remaining open before cutover: FX-073 (PARTIAL — relieved by the 0.015 retune), FX-076, FX-064, FX-075, FX-077, FX-062. |
...
### 10.2 + 10.3 — Known-fixed bugs + Retired components

**Moved 2026-05-28** to compact the arch doc:

- **§10.2 (Known-fixed bugs)** removed — duplicated `Polymarket bot fixit.md`
  §4 "Fixed issues" (live single-source-of-truth). Use fixit.md for the
  per-entry history of each FX-NNN fix; this section had become a stale
  mirror requiring double-update on every ship cycle.

- **§10.3 (Known limitations / retired Patch-1–13 components)** moved to
  `CHANGELOG.md` as the appendix "Historical: Retired components (v4.0 era)".
  Describes the deleted Bandit / LearningController / β-η stack that was
  removed in v4.0 → v5.0. Not relevant for understanding the current
  system; useful only for archaeologists.
...
### 11.4 — 11.14 (deployment procedures)

**Moved to `docs/runbooks/deployment_from_scratch.md`** on 2026-05-28 to
keep the architecture doc focused on the system spec (the WHAT and WHY)
rather than procedural steps (the HOW). Procedures will drift as Ubuntu
and Hetzner versions change; runbooks are the right place for them.

Topics in the runbook:
- §11.4 Server provisioning (Hetzner Cloud)
- §11.5 Server hardening
- §11.6 Install Python 3.14 + build tools
- §11.7 GitHub deploy key
- §11.8 Clone repo + Python deps
- §11.9 Transfer `.env`
- §11.10 Smoke tests
- §11.11 Install systemd units
- §11.12 DRY soak before LIVE
- §11.13 LIVE cutover
- §11.14 Operational lifecycle commands

For day-to-day operations after the bot is running (mode/stage
promotion, G1 7-day monitoring), see
`docs/runbooks/9_of_10_p5_p7_operator_runbook.md`.
