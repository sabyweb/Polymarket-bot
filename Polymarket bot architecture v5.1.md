...
| **Server deployment** | Hetzner CCX13 in Helsinki (`hel1`), Ubuntu 24.04 LTS, Python 3.14.4. **(unverified: whether Helsinki has pulled latest repo HEAD; bot has been halted since 2026-05-25, so likely on `0fafa1b` or earlier. Operator to `git pull origin main` as part of P5.)** Operator runbook for fresh provisioning at `docs/runbooks/deployment_from_scratch.md`; for cutover/lifecycle at `docs/runbooks/9_of_10_p5_p7_operator_runbook.md`. | §11.1–11.3 (current); `docs/runbooks/` (procedures) |
| **Wallet** | FUNDER `0xB23Bc80E6719099aeBE0c34389f05EC8C928503f` (Polymarket proxy). pUSD balance **(unverified — last known $226.09 on 2026-05-24; bot has been halted since 2026-05-25 so balance is approximately static modulo passive rewards if any)**. All V2 allowances unlimited, CTF approvals True. FX-049 wallet reconciliation invariant remains armed for next cycle after rebring-up. | `check_wallet.py`, `wallet_reconcile_history` table |
