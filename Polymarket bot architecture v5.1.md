...
## Current Production State (v6.1 — 9/10 plan code-level phases shipped; bot halted on Helsinki pending live rebring-up)
...
| **Fill-detection (FX-054, P-pre-session)** | 3-axis defensive design: **F1** idempotent `log_fill` with partial unique index on `fill_event_id` + truthful bool return + honest `[FILL_WRITE]` log step. **F2** phantom-check fail-OPEN within `FILL_BALANCE_LAG_TOLERANCE_SEC=60` of `slot.placed_at` (closes Polygon CTF-confirmation-lag false phantoms). **F3** end-of-cycle drift catch-up sweep on `(cids_processed − primary_handled)` with 5-min-bucketed event_id idempotency. Symmetric SELL-side FX-007 still applies. **(unverified in production against a real fill burst — gate G-C pending operator P5/P6.)** | `order_lifecycle.py:detect_fills + _check_buy_phantom_fill + _reconcile_balance_drift`, `database.py:log_fill` |
| **Wallet reconciliation (FX-049 + FX-055)** | Runs once per `simple_oversight.run_once` cycle via `oversight/wallet_reconciliation.py::reconcile_wallet_invariant`. Compares actual wallet pUSD delta against expected (`Σ unwinds − Σ fills + data-api REWARD + MAKER_REBATE` since last baseline). `\|divergence\| > RF_WALLET_DESYNC_THRESHOLD_USD = $0.50` → `[CRITICAL] WALLET_DESYNC`. First-run baseline + fail-OPEN on data-api failure preserved from FX-049. FX-055 re-wired this into simple_oversight after the 2026-05-25 regression dropped it. | `oversight/wallet_reconciliation.py`, `simple_oversight.py:run_once`, `wallet_reconcile_history` table |
...
| **Oversight evaluator (current path)** | `simple_oversight.py::run_once` — 30-min cycle. Sequence: wallet probe → portfolio_snapshots write → peak/24h history reads → FX-055 wallet reconciliation → FX-051 tracker.tick + policy.evaluate → q_share divergence record (P11) → `allocator.compute()` with 6 inputs (wallet, peak, 24h_ago, realized_loss, + the 5 policy outputs above) → write alloc.json → telemetry. All in fail-open try/excepts so a single subsystem failure can't kill the cycle. | `simple_oversight.py:run_once` |
